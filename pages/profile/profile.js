    ...
    return [
        ...
        },
        {
            ...
            items: [
                ...
                { id: 'feedback', icon: '💬', iconType: 'bell', title: '意见反馈', desc: '通过微信客服反馈', action: 'tap' }
            ]
        },
        {
            title: '开发者选项',
...
Page({
    ...
    async onUpgrade() {
        ...
        const payRes = await new Promise(resolve => {
            // 参数对齐虚拟支付官方签名约定
            wx.requestVirtualPayment(Object.assign(Object.assign({}, params), { mode: 'short_series_goods', success: () => resolve({ ok: true }), fail: (err) => resolve({ ok: false, err: (err === null || err === void 0 ? void 0 : err.errMsg) || '支付失败' }) }));
        });
        if (!payRes.ok) {
     ...
     */
    async pollOrderUntilPaid(outTradeNo) {
    ...
    getOrderCapped(outTradeNo, capMs) {
        return Promise.race([
            getOrder(outTradeNo),
            new Promise((_, reject) => setTimeout(() => reject(new Error('ORDER_QUERY_TIMEOUT')), capMs))
        ]);
    },
    // ========== 我的昵称 ==========
    /**
