...
function buildMenuSections(tier: Tier, windowDates: number, isLoggedIn: boolean): { title: string; items: MenuItem[] }[] {
  ...
  const accountSection = isLoggedIn
    ? {
        title: '账号',
        items: [
          { id: 'logout', icon: '🚪', iconType: 'warning' as const, title: '退出登录', desc: '下次使用需重新登录', action: 'tap' as const }
        ]
      }
    : {
        title: '账号',
  ...
  onSync() {
    ...
    wx.showModal({
      ...
      success: async (res) => {
        ...
        if (r.failed > 0) lines.push(`${r.failed} 条失败（联网后会再试）`);
        if (r.trimmed > 0) lines.push(`更早的 ${r.trimmed} 条没备份（免费版只保留最近几天）`);
