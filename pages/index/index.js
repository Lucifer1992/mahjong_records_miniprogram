...
Page({
    ...
    onLoad() {
        ...
        if (idx >= 0) {
            this.setData({
                ...
                selectedRule: this.data.rules[idx]
            });
        }
    ...
    onRuleChange(e) {
        ...
        if (picked.id === 'custom') {
            ...
            setTimeout(() => {
                var _a, _b, _c, _d;
                (_d = (_c = (_b = (_a = this).selectComponent) === null || _b === void 0 ? void 0 : _b.call(_a, '#customRuleInput')) === null || _c === void 0 ? void 0 : _c.focus) === null || _d === void 0 ? void 0 : _d.call(_c);
            }, 0);
            return;
        }
        // 切回预设：
    ...
    refreshRecentPlayers() {
        ...
        const withUsage = sorted
            .map(p => (Object.assign(Object.assign({}, p), { usage: usage.get(p.nickname) || 0, inGame: false, checked: false })))
        ...
        const list = withUsage.map(p => (Object.assign(Object.assign({}, p), { inGame: inGameIds.has(p.id), checked: inGameIds.has(p.id) })));
        this.setData({
            recentPlayers: list,
     ...
     */
    setRecentChecked(ids) {
        const set = new Set(ids);
        const list = this.data.recentPlayers.map(p => (Object.assign(Object.assign({}, p), { checked: p.inGame || set.has(p.id) })));
        this.setData({
            recentPlayers: list,
    ...
    onConfirmAddRecent() {
        ...
        for (const p of picked) {
            if (players.length >= MAX_PLAYERS)
                break;
            if (players.some(x => x.playerId === p.id))
                continue;
            players.push({
                playerId: p.id,
                nickname: p.nickname,
    ...
    async onSave() {
        ...
        if (this.data.selectedRule.id === 'custom' && !this.data.customRuleDraft.trim()) {
            ...
            // 自动聚焦输入框
            setTimeout(() => { var _a, _b, _c, _d; return (_d = (_c = (_b = (_a = this).selectComponent) === null || _b === void 0 ? void 0 : _b.call(_a, '#customRuleInput')) === null || _c === void 0 ? void 0 : _c.focus) === null || _d === void 0 ? void 0 : _d.call(_c); }, 0);
            return;
        }
        ...
        addRecord(record);
        // 记住本次玩法，下次进首页时默认选中
        rememberLastRuleType(this.data.selectedRule.id);
        // 记住本次时段，下次进首页时优先用它（无历史时仍按本机时间推断）
        rememberLastDuration(this.data.selectedDuration.id);
