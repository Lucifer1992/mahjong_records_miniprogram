...
export function getSolarTerm(date) {
    ...
    // 该月的两个节气索引
    const first = (m - 1) * 2;
    if (termDay(y, first) === d)
        return SOLAR_TERMS[first];
    if (termDay(y, first + 1) === d)
        return SOLAR_TERMS[first + 1];
    return '';
}
/** 当日传统节日名（不是节日则返回空串） */
export function getFestival(lunar) {
    if (lunar.isLeap)
        return ''; // 闰月不算节日
...
export function getFullLunarText(date) {
    const lunar = toLunar(date);
    const festival = getFestival(lunar);
    const term = getSolarTerm(date);
