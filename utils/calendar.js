 ...
 */
export function buildCalendar(records, year, month, selfPlayerId) {
    const cells = [];
    // 当月第一天是星期几（0=周日）
    ...
    for (const record of records) {
        ...
        const isInThisMonth = new Date(record.playedAt).getFullYear() === year &&
    ...
    for (const cell of dayMap.values()) {
        if (cell.netScore > 0)
            cell.result = 'win';
        else if (cell.netScore < 0)
            cell.result = 'lose';
        else if (cell.gamesPlayed > 0)
            cell.result = 'even';
    }
    // 填充日历单元格
    // 前导空白
    ...
    for (const cell of dayMap.values()) {
        ...
        if (cell.result === 'win')
            winDays++;
        else if (cell.result === 'lose')
            loseDays++;
        else if (cell.result === 'even')
            evenDays++;
    }
    return {
        year,
