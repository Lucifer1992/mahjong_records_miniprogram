...
import { getTier, setTier, setFreeWindowDates } from './tier';
 ...
 * 加入待上传队列（写入战绩后调用）
 */
export function enqueuePush(recordId) {
 ...
 * 加入待删除队列
 */
export function enqueueDelete(recordId) {
 ...
 */
export async function syncFull(records) {
    ...
    for (let i = 0; i < records.length; i += CHUNK) {
        ...
        try {
            ...
            if (res.tier)
                tier = res.tier;
            if (typeof res.trimmed === 'number')
                trimmed += res.trimmed;
            res.results.filter(r => !r.ok && r.id).forEach(r => failedIds.push(r.id));
        }
        catch (_b) {
            // 整批失败：这些记录全部算失败，重新入队
            failed += chunk.length;
 ...
 */
export async function pullAndMerge(localRecords) {
    const fallback = getTier();
    try {
        ...
        for (const [id, cloud] of cloudMap) {
            if (!localMap.has(id))
                toPull.push(normalizeRecord(cloud));
        }
 ...
 * 用户无感；失败也不打扰
 */
export function tryAutoSync(records) {
    // 延迟 2 秒，避开页面刚加载的请求高峰
    setTimeout(() => {
 ...
 * - 否则静默
 */
export async function bootSync(localRecords) {
