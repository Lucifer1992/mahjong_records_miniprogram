 ...
 */
export function analyzeFortune(records, playerId) {
    if (records.length < MIN_GAMES_FOR_ANALYSIS) {
        return null;
    }
    // 1. 收集与目标玩家同桌的所有场次
    const relevantRecords = records.filter(r => r.players.some(p => p.playerId === playerId));
 ...
 * 格式化净胜分
 */
export function formatNetScore(score) {
    if (score > 0)
        return `+${score}`;
    return String(score);
}
