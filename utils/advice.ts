...
import { GameRecord, GameDuration, PartnerStat } from './types';
 ...
 */
export function buildMonthAdvice(
  records: GameRecord[],
  selfPlayerId: string
): MonthAdvice {
  const good: AdviceItem[] = [];
  const bad: AdviceItem[] = [];
  ...
  if (fortune) {
    const pick = (list: PartnerStat[]): PartnerStat | undefined =>
      list.find(p => p.gamesTogether >= MIN_SAMPLE);
  ...
  // ---- 维度 2：时段 ----
  const statMap = new Map<GameDuration, DurationStat>();
  for (const r of mine) {
    ...
    cur.games += 1;
    if (self.score > 0) cur.wins += 1;
    cur.net += self.score;
    statMap.set(r.duration, cur);
  }

  const qualified: DurationStat[] = DURATIONS
    .map(d => {
      const st = statMap.get(d.id as GameDuration);
      return {
        id: d.id as GameDuration,
        name: d.name,
        games: st?.games || 0,
        wins: st?.wins || 0,
        net: st?.net || 0
