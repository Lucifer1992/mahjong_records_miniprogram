 ...
 * 保证总和精确等于 0
 */
function buildScores(rng, table, bias, idOf) {
...
// ========== 玩家档案统计 ==========
function scoreOf(record, playerId) {
    const entry = record.players.find(p => p.playerId === playerId);
    return entry ? entry.score : null;
}
function applyPlayerStats(players, records) {
...
export function generateMockData(seed = 20260913) {
    ...
    for (let back = DAYS_BACK; back >= 0; back--) {
        ...
        const duration = roll < 0.10 ? 'morning' :
            roll < 0.30 ? 'afternoon' :
                roll < 0.90 ? 'evening' : 'overnight';
        const startHour = duration === 'morning' ? 9 :
        ...
        for (let k = 0; k < rounds; k++) {
            ...
            let bias = 0;
            if (table.indexOf(LUCKY.nickname) >= 0)
                bias += LUCKY.bias;
            if (table.indexOf(EVIL.nickname) >= 0)
                bias += EVIL.bias;
 ...
 * 便于「恢复生成前数据」回滚
 */
export function loadMockData(seed = 20260913) {
    try {
