...
export function adUnitId(slot: AdSlot): string {
 ...
 */
export function adEnabled(slot: AdSlot, isProUser: boolean): boolean {
...
function getRewardedAd(): any {
  if (!AD_UNITS.rewardedVideo) return null;
...
/** 授予某项解锁（默认 24 小时有效） */
export function grantAdUnlock(key: string, durationMs: number = 24 * 3600 * 1000): void {
...
export function isAdUnlocked(key: string): boolean {
  const expireAt = readUnlocks()[key];
  if (!expireAt) return false;
  if (Date.now() > expireAt) return false; // 过期视为未解锁
  return true;
}
